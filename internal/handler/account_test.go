...
func TestExtractPreferredUsernameFromRequest_IDTokenOverHeader(t *testing.T) {
	header := base64.RawURLEncoding.EncodeToString([]byte(`{"alg":"RS256","typ":"JWT"}`))
	payload := base64.RawURLEncoding.EncodeToString([]byte(`{"preferred_username":"FertoKitty"}`))
	signature := base64.RawURLEncoding.EncodeToString([]byte("fakesig"))
	jwtToken := header + "." + payload + "." + signature

	req := httptest.NewRequest(http.MethodGet, "/", nil)
	req.Header.Set("X-Forwarded-Preferred-Username", "becci")
	req.Header.Set("Authorization", "Bearer "+jwtToken)

	got := handler.ExtractPreferredUsernameFromRequest(req)
	if got != "FertoKitty" {
		t.Errorf("expected FertoKitty from ID token, got %q", got)
	}
}
